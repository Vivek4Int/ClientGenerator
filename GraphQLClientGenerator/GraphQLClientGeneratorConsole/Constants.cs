...
public static readonly string  queryToFetchGrqphQLSchema = "{\"query\":\"query IntrospectionQuery {   "+
                ...
                "args {          ...InputValue        }      }    }  }  fragment FullType "+
                "on __Type {    kind    name    description    fields(includeDeprecated: true)"+
                ...
                "  isDeprecated      deprecationReason    }    possibleTypes {      ...TypeRef "+
                "   }  }  fragment InputValue on __InputValue {    name    description    "+
                "type { ...TypeRef }    defaultValue  }  fragment TypeRef on __Type {   "+
                " kind    name    ofType {      kind      name      ofType {        kind  "+
                "      name        ofType {          kind          name            ofType {  "+
                "          kind            name            ofType {              kind          "+
